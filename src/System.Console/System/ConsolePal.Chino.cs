...
namespace System
{
    ...
    internal static class ConsolePal
    {
        ...
        {
            return new ChinoConsoleStream(Interop.IO.StdinHandle, FileAccess.Read);
        }

        public static Stream OpenStandardOutput()
        {
            return new ChinoConsoleStream(Interop.IO.StdoutHandle, FileAccess.Write);
        }

        public static Stream OpenStandardError()
        {
            return new ChinoConsoleStream(Interop.IO.StderrHandle, FileAccess.Write);
        }
        ...
        {
            return !Interop.IO.IsConsoleHandle(fd);
        }
        ...
        {
            return IsHandleRedirected(Interop.IO.StdinHandle);
        }
        ...
        {
            return IsHandleRedirected(Interop.IO.StdoutHandle);
        }
        ...
        {
            return IsHandleRedirected(Interop.IO.StderrHandle);
        }
        ...
        private static void EnsureInitializedCore()
        {
            lock (Console.Out) // ensure that writing the ANSI string and setting initialized to true are done atomically
            {
                if (!s_initialized)
                {
                    ...
                    s_veraseCharacter = (byte)'\x08';
                    s_veolCharacter = (byte)'\0';
                    s_veol2Character = (byte)'\0';
                    s_veofCharacter = (byte)'\x04';
        ...
        {
            return Interop.IO.Read(fd, new Span<byte>(buffer, offset, count));
        }
        ...
        private static unsafe void Write(SafeFileHandle fd, byte* bufPtr, int count, bool mayChangeCursorPosition = true)
        {
            if (count > 0)
            {
                ...
                Interop.IO.Write(fd, new ReadOnlySpan<byte>(bufPtr, count));

                if (mayChangeCursorPosition)
                {
                    UpdatedCachedCursorPosition(bufPtr, count, cursorVersion);
                }
            }
        }
        ...
        internal static unsafe void WriteStdoutAnsiString(string? value, bool mayChangeCursorPosition = true)
        {
            ...
            if (value.Length <= StackAllocThreshold)
            {
                ...
                fixed (char* chars = value)
                {
                    ...
                    lock (Console.Out) // synchronize with other writers
                    {
                        Write(Interop.IO.StdoutHandle, data, bytesToWrite, mayChangeCursorPosition);
                    }
                }
            }
            else
            {
                ...
                lock (Console.Out) // synchronize with other writers
                {
                    Write(Interop.IO.StdoutHandle, data, 0, data.Length, mayChangeCursorPosition);
                }
            }
        }

        /// <summary>Provides a stream to use for Chino console input or output.</summary>
        private sealed class ChinoConsoleStream : ConsoleStream
        {
            ...
            internal ChinoConsoleStream(SafeFileHandle handle, FileAccess access)
                : base(access)
            {
